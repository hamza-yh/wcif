...
async function fetchWcaIds() {
  for (let i = 0; i < 3; i++) {
    const { data, error } = await supabase.from('users').select('wca_id');
    if (!error && data) {
      console.log("✅ Supabase fetch successful");
      return new Set(data.map(u => u.wca_id));
    }
    console.error(`❌ Supabase attempt ${i + 1} failed:`, error?.message || "no data");
  }
  console.error("🚨 Supabase fetch failed after 3 attempts. Exiting.");
  process.exit(1);
}
...
async function main() {
  ...
  const wcaIds = await fetchWcaIds();
  ...
  for (const id of ids) {
    ...
    const trimmed = {
      ...
      registrations: wcif.persons.filter(person => person.registration && person.registration.isCompeting).map(person => person.wcaId).filter(id => id), // filter out undefined
    }
    comps.push(trimmed)
    ...
    if (prevRegState === 'notOpen' && newRegState === 'open' || prevRegState === 'open' && newRegState === 'closing') {
